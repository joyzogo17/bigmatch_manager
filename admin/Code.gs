...
function doPost(e) {
  try {
    ...
    switch (action) {
      ...
      case 'clearMatches': result = apiClearMatches(role);      break;  // v1.0.6
      default:            result = { error: 'Action inconnue', code: 400 };
    }
    return corsOutput(result);
  } catch (err) {
...
function apiAddMatch(data, role) {
  ...
  getSheet(SH_MATCHS).appendRow([
    ...
    // Traçabilité v1.0.5
    data.scoredBy     || role,
    data.scoredByTeam || role,
    now,   // horodatage score
    now,   // dernière modification
    data.scoredBy || role,
    data.eq1 || ''  // équipe domicile = eq1
  ]);
...
// ── POST: CLEAR ALL MATCHES (v1.0.6) ─────────────────────────────
/**
 * apiClearMatches(role)
 * Vide la feuille MATCHS en conservant l'en-tête.
 * Réservé au Contrôleur uniquement.
 */
function apiClearMatches(role) {
  if (role !== 'Contrôleur') {
    return { error: 'Seul le Contrôleur peut réinitialiser les données', code: 403 };
  }
  const sheet = getSheet(SH_MATCHS);
  ...
  addLog(role, 'clearMatches', 'Réinitialisation complète de la feuille MATCHS');
  return { ok: true, message: 'Feuille MATCHS réinitialisée' };
}
...
function initSpreadsheet() {
  const sheetDefs = [
    { name: SH_MATCHS,  headers: ['ID','Date','Jour','Heure','Type','Statut','Motif','Equipe1','Equipe2','Score1','Score2','Buteurs1','Buteurs2','Annee','Verrouille','EventID','SaisiPar','SaisiParEquipe','HorodatageScore','DerniereModif','ModifiePar','EquipeDomicile'] },
    ...
    { name: SH_USERS,   headers: ['ID','Nom','Role','Type','DateCreation','DerniereConnexion','CreePar'] },
  ...
  SpreadsheetApp.getUi().alert('Big Match Manager v1.0.6 Cygnus\nFeuilles initialisees avec succes !\nColonnes de tracabilite et derniere connexion ajoutees.');
}
